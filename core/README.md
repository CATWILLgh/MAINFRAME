...
3. Commit sources with the committed render outputs and golden fixtures they affect. OpenCode agents and most Codex/Antigravity native projections are regenerated locally and remain gitignored.
...
user-owned in that file. OpenCode, Codex, and Antigravity native projections
...
OpenCode/Codex/Antigravity golden fixtures remain direct-owned tests, not additional
artifact layers.
...
Codex, and Antigravity builder test suites separately guard their native
...
- `gates/` — gate detectors (Python) + their data; wire contract in
  ...
  bridge generated from `adapters/antigravity-2/`.
...
- `instructions/` — the umbrella behavioral rules as ordered section
  ...
  `${CODEX_HOME:-~/.codex}/AGENTS.md` by `install.sh --codex`). Antigravity
