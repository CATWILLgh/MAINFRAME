...
Per-ORM: TypeORM `@Index()` + `repo.upsert()` or qb `.orUpdate()`; Prisma `@@index` + `upsert()`; Drizzle `index()` / `uniqueIndex()` + `.onConflictDoUpdate()`. JSONB `@>` usually needs raw SQL in the query builder.
