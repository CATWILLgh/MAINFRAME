...
- **Model + effort (`sonnet` / `medium`) are calibrated via 6-variant × 10-round tournament (ADR 0062).** Winner picked on plan reasoning + execution combined: perfect-run rate 8/10 (vs sonnet-low 4/10), avg quality 2.70/3.00, 100% Phase-H format compliance, zero errors. Re-tournament after a notable prompt-body change.
