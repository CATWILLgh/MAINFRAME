...
- **Model + effort (`sonnet` / `medium`) are calibrated via 6-variant × 10-round tournament (ADR 0063).** Winner picked on plan reasoning + execution combined: perfect-run rate 9/10 (vs sonnet-low 8/10 + lower plan quality), avg quality 2.90/3.00, perfect exec quality 3.00, 100% Phase-H format compliance, zero errors. Re-tournament after a notable prompt-body change.
