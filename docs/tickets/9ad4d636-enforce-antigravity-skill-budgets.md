...
**Where:** generated `skills/react-frontend-patterns/forms.md`
**Additional details:** Full validation of the merged projection reports only the existing `SUPP-LINES` warning at 61 non-empty lines. The earlier `task-workflow/SKILL.md` token warning no longer reproduces after the current core and projection changes. The remaining supporting-file budget violation stays in scope for this ticket and does not block the branch merge.
