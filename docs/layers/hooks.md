...
Live registration: `adapters/claude-code/plugin/hooks/hooks.json` (source of truth). The hub uses **8 of the 30 events**:

| Event | Matcher | Hub scripts |
|---|---|---|
| `SessionStart` | `startup\|resume\|clear\|compact` | `hooklib-smoke-check`, `telemetry`, `concise-reminder` (resets the per-session turn counter), `init-reminder` (keeps activation on resume, resets its counter after compact, deactivates on startup or `/clear`) |
...
| `PreToolUse` | `Bash` | `path-validation`, `secret-commit-gate` (deny `git commit` that stages a high-confidence vendor token; scans `git diff --cached`/`HEAD`, skips SOPS/git-crypt repos, fail-safe defers; ADR 0079), `bash-pattern-reminder` |
| `PreToolUse` | `Skill` | `telemetry` |
| `PreToolUse` | `TodoWrite` | `telemetry` (`todo_write` event — counts only: total + per-status `pending`/`in_progress`/`completed`, never the todo text) |
...
| `Stop` | `*` | `stop-gate-suppression-markers`, `stop-gate-comment-discipline` (process-narration comments vs HEAD, shared `_markers.flag_comment`), `python-security-stop-gate`, `nodejs-security-stop-gate`, `frontend-fsd-gate`, `frontend-dead-code`, `fallow-quality-note` (advisory: fallow analyzer на изменённых TS/JS; throttled 5 min; conservative categories only), `length-quality-note` (advisory, never blocks: flags changed files over 400 lines — `_length_check.FILE_LENGTH_EXTENSIONS`, excludes `.sql`/`.vue`/`.svelte`, which legitimately run long for structural reasons unrelated to hand-written logic — and Python functions over 60 lines via a recursive `ast.NodeVisitor`-built qualname, per `adapters/claude-code/export/CLAUDE.md`'s length rule; no delta/inherited split — decision-reviewer + advisor 2026-07-06 found the before/after size comparison bought one thin benefit at three real costs given advisory-only severity, so a per-file ticket under `docs/tickets/` suppresses BOTH checks uniformly instead; no throttle, unlike `fallow-quality-note`, since this is pure local line-count + `ast.parse`, not an expensive external analyzer call), `memory-reminder` (advisory, non-blocking, main-session-only via absent `agent_id`: nudges the model to persist durable facts to native auto-memory + folds in a `harness-feedback` mention; throttled independently per `session_id` for 30 min, skips trivial sessions and malformed payloads without a session id; replaces the retired `feedback-nudge-compact`; ADR 0080), `concise-reminder` (advisory, non-blocking, main-agent-only: every N main-agent turns (default 8, env `MAINFRAME_CONCISE_REMINDER_EVERY`) re-anchors the active output style's concise+plain rule near the recent tail — the rule sits in the system prompt every turn but adherence drifts on long runs (recency), and a custom file output style's built-in `turnReminder` is only a generic stub; logs `concise_reminder` every turn (`reply_chars` + `turns_since_reminder` distance — Stop fires after the reply, so the fire-turn reply is untreated at max distance and the first treated reply is the next turn at 1; the metric is the distance curve, not a fire-turn boolean) to measure effect; per-session turn counter reset on SessionStart incl. compact; pilot ADR 0084), `init-reminder` (advisory and main-session-only: after manual `/mainframe:init`, re-anchors a short primary-session contract every 64 completed primary turns; `agent_id` payloads are rejected before state access, so subagents neither count nor receive it; env `MAINFRAME_INIT_REMINDER_EVERY` permits empirical retuning) — on dev installs every `emit_block` reason carries the `harness-feedback` nudge (`_hooklib.FEEDBACK_NUDGE`; silent on plain installs) |
| `PermissionDenied` / `SubagentStart` / `SessionEnd` | `*` | `telemetry` |

Shared scaffolding (`_hooklib.py` + `_markers.py`, stdlib-only) underlies them. The other 22 events are unused — see the opportunity map below.
