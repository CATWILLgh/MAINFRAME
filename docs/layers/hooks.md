...
Live registration: `plugin-dist/hooks/hooks.json` (source of truth). As of 2026-06-12 the hub uses **7 of the 30 events**:

| Event | Matcher | Hub scripts |
|---|---|---|
| `SessionStart` | `startup\|resume\|clear\|compact` | `session-posture`, `hooklib-smoke-check`, `telemetry`, `task-workflow-engagement` (resets the per-segment engagement marker) |
