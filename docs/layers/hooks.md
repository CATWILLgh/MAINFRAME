...
- Shared source: detectors in `core/gates/detectors/` and gate data in `core/gates/rules/`.
...
**vs the stale 16-event May list:** the running CLI confirms **exactly 30**. Newly surfaced and hub-relevant: `PermissionDenied` (#5 — fires when the auto-mode classifier blocks a tool → a direct, countable signal of **auto-mode friction**, the user's primary workflow), `UserPromptExpansion` (#8 — slash / skill expansion), `MessageDisplay` (#30). The hub currently registers **34 hooks across 7 of the 30 events** (`SessionStart` 5, `PreToolUse` 8, `PostToolUse` 8, `Stop` 10, plus `PermissionDenied` / `SubagentStart` / `SessionEnd` 1 each), verified from the authored `adapters/claude-code/gates/hooks.json` on 2026-07-14.
...
- **Two channels for attaching a hook to a subagent:**
  1. **Global** — authored in `adapters/claude-code/gates/hooks.json` and rendered to `dist/claude-code/plugin/hooks/hooks.json` (Claude Code also accepts global hooks in `~/.claude/settings.json`, which the hub no longer uses for hooks). Fires for all: main agent + every subagent.
...
Authored registration: `adapters/claude-code/gates/hooks.json`; `dist/claude-code/plugin/hooks/hooks.json` is its rendered copy. As verified on 2026-07-14, the hub uses **7 of the 30 events**:

| Event | Matcher | Hub scripts |
|---|---|---|
| `SessionStart` | `startup\|resume\|clear\|compact` | `session-posture`, `hooklib-smoke-check`, `telemetry`, `task-workflow-engagement` (resets the per-segment engagement marker), `concise-reminder` (resets the per-session turn counter) |
...
| `PreToolUse` | `Skill` | `telemetry`, `task-workflow-engagement` (marks the segment `active` when `task-workflow` is invoked) |
| `PreToolUse` | `Edit\|Write\|MultiEdit` | `task-workflow-engagement` (advisory, main-agent-only: on the first file-modify of a segment where `task-workflow` was not invoked, nudge to invoke it — a skill survives compaction via re-attachment of its first 5K tokens but is deprioritised and framed "context only / do not re-execute", so present ≠ followed; re-invoke re-activates. Marker reset on every SessionStart incl. compact; fires once per segment) |
| `PreToolUse` | `TodoWrite` | `telemetry` (`todo_write` event — counts only: total + per-status `pending`/`in_progress`/`completed`, never the todo text. Measures the task-workflow todo-checklist pilot: does seeding a per-task checklist get adopted, and do dropped-step incidents fall) |
| `PostToolUse` | `Edit\|Write\|MultiEdit` | `scan-suppression-markers`, `comment-discipline-reminder`, `ticket-id-format-reminder` (advisory: a Write of `docs/tickets/<NNN>-*.md` with a sequential id — surface-ticket allocates random 8-hex ids `openssl rand -hex 4`, branch-collision-free; the skill warns but agents revert to legacy NNN by pattern-matching existing tickets, so this is the mechanical backstop; Write-only, fires on new tickets not legacy edits), `python-security-scan`, `python-deps-audit`, `nodejs-deps-audit`, `nodejs-security-scan`, `telemetry` (ticket-creation rate + `code_edit` domain bucket with `agent_type` attribution — denominator for profile-agent under-use) |
| `Stop` | `*` | `stop-gate-suppression-markers`, `stop-gate-comment-discipline` (process-narration comments vs HEAD, shared `_markers.flag_comment`), `python-security-stop-gate`, `nodejs-security-stop-gate`, `frontend-fsd-gate`, `frontend-dead-code`, `fallow-quality-note` (advisory: fallow analyzer on changed TS/JS; throttled 5 min; conservative categories only), `length-quality-note` (advisory, never blocks: flags changed files over 400 lines — `_length_check.FILE_LENGTH_EXTENSIONS`, excludes `.sql`/`.vue`/`.svelte`, which legitimately run long for structural reasons unrelated to hand-written logic — and Python functions over 60 lines via a recursive `ast.NodeVisitor`-built qualname, per the authored length rule in `core/instructions/50-engineering-practices.md`; no delta/inherited split — decision-reviewer + advisor 2026-07-06 found the before/after size comparison bought one thin benefit at three real costs given advisory-only severity, so a per-file ticket under `docs/tickets/` suppresses BOTH checks uniformly instead; no throttle, unlike `fallow-quality-note`, since this is pure local line-count + `ast.parse`, not an expensive external analyzer call), `memory-reminder` (advisory, non-blocking: nudges the model to persist durable facts to native auto-memory + folds in a `harness-feedback` mention; throttled 30 min, skips trivial sessions; replaces the retired `feedback-nudge-compact`; ADR 0080), `concise-reminder` (advisory, non-blocking, main-agent-only: every N main-agent turns (default 8, env `MAINFRAME_CONCISE_REMINDER_EVERY`) re-anchors the active output style's concise+plain rule near the recent tail — the rule sits in the system prompt every turn but adherence drifts on long runs (recency), and a custom file output style's built-in `turnReminder` is only a generic stub; logs `concise_reminder` every turn (`reply_chars` + `turns_since_reminder` distance — Stop fires after the reply, so the fire-turn reply is untreated at max distance and the first treated reply is the next turn at 1; the metric is the distance curve, not a fire-turn boolean) to measure effect; per-session turn counter reset on SessionStart incl. compact; pilot ADR 0084) — on dev installs every `emit_block` reason carries the `harness-feedback` nudge (`_hooklib.FEEDBACK_NUDGE`; silent on plain installs) |
| `PermissionDenied` / `SubagentStart` / `SessionEnd` | `*` | `telemetry` |

Shared scaffolding (`_hooklib.py` + `_markers.py`, stdlib-only) underlies them. The other 23 events are unused — see the opportunity map below.
