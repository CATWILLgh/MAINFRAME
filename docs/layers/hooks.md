...
Live registration: `adapters/claude-code/plugin/hooks/hooks.json` (source of truth). The hub uses **9 of the 30 events**:
...
Shared scaffolding (`_hooklib.py` + `_markers.py`, stdlib-only) underlies them. The other 21 events are unused — see the opportunity map below.
