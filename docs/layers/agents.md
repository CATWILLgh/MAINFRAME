...
4. ✓ **RESOLVED (2026-06-02).** A `disable-model-invocation: true` skill preloaded via sub-agent `skills:` **does** load inside the sub-agent, and is **not** auto-loadable in the main context — the two are independent axes (model-invocation gating ≠ startup injection). Verified empirically via `claude -p` probes on `devops-engineer` + `dokploy-api`: the agent reproduced skill-distinctive content (the `x-api-key` / `trpc-to-openapi`-artifact auth gotcha), while a fresh main session reported the skill unavailable to it and correctly pointed at the agent. Confirms bloat-prevention #3 in [decision-tree.md](decision-tree.md). Caveat observed: under a terse forced-recall prompt the agent can still confabulate a detail it holds in-context (stated the misleading label as `bearerAuth` rather than `Authorization`) — preload presence does not guarantee precise recall; real work should consult the loaded file, not paraphrase it.
