...
### 1.6. Скиллы в субагенте — доступ vs preload (находка 2026-06-01)

Две **ортогональные** оси (источник: `code.claude.com/docs/en/sub-agents`). Это не про «другой движок» — движок тот же, гейтит `tools`:

- **Доступ** (может ли субагент *вызвать* скилл) — через `Skill`-инструмент:
  - `tools:` указан и **не содержит `Skill`** → вызвать скилл **нельзя**.
  - `tools:` **опущен** → наследует все tools, включая `Skill` → может вызвать любой project/user/plugin скилл.
  - Запретить совсем: убрать `Skill` из `tools` **или** добавить в `disallowedTools`.
- **Preload** (`skills:` во frontmatter) — впрыскивает **полный контент** перечисленных скиллов в контекст на старте, минуя семантический матч. Дословно: *«controls which skills are preloaded, not which skills the subagent can access»*.

**Следствие:** субагент без `Skill` в `tools` и без нужного скилла в `skills:` **не достанет** этот скилл никак — ни авто-матчем, ни вызовом. Поэтому напоминающий хук должен нести знание **inline**, а не «сходи прочитай скилл» (сабагент может физически не мочь). Все наши хаб-агенты — с узким `tools:` без `Skill`, доступ к скиллам у них только через `skills:` preload.
...
4. ✓ **RESOLVED (2026-06-01).** Sub-agent `skills:` preload верифицирован эмпирически (хаб-агенты `decision-reviewer` / `*-engineer` стартуют с preloaded скиллами и работают). Механика доступа vs preload уточнена — см. §1.6.
