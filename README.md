...
If both subscriptions are available, add the optional peer-work bridge to the
adapter that should call the other CLI:

```bash
./install.sh --claude --with-peer  # Claude may ask Codex to review or implement
./install.sh --codex --with-peer   # Codex may ask Claude to review or implement
```
...
the managed peer skill, launcher, and completion bridge. A peer session may
review or implement one bounded result. Corrections to that result resume its
exact session; a new result starts a new session. The calling agent still owns
...
The same page shows the durable local analysis queue and can run or pause the optional Spark and Antigravity review workers. In Codex development mode, completed subagents also enter a separate retryable Antigravity queue: the hook only records a private local job, Observatory removes tool arguments, results, paths, secret-shaped text, and the actual runtime model before analysis, then attaches observed runtime metadata to the local review artifact. Their output remains review-only; it never changes model selection, adapter policy, or project code.
