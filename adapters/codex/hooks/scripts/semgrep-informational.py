...
try:
    from _hooklib import emit_note, load_payload, log_event, log_hook_signal, run
...
def _record(
    payload: dict, status: str, duration_ms: int, files: int, findings: int
) -> None:
    log_event(
        "analyzer_run",
        {
...
def main() -> None:
    ...
    try:
        ...
        _record(payload, "unavailable", duration, len(targets), 0)
        if claim_once(
        ...
        duration = max(0, round((time.monotonic() - started) * 1000))
        _record(payload, "failed", duration, len(targets), 0)
        if claim_once(
    ...
    _record(payload, "completed", duration, len(targets), matched_findings)
    if not rows:
        return
    shown = rows[:MAX_ROWS]
    ...
    for rule_id, count in counts.items():
        log_hook_signal(__file__, rule_id, "noted", count, payload, context=note)
