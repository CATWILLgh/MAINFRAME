...
- `execution: user-approved` means the user resolved the recorded choice through
  `init ticket <id>`. It may be implemented only by that route's exact one-ticket
  goal, never by the queue-wide autonomous implementation run.
...
At the start of `tickets-find`, bring current open ticket records under the
canonical tree without asking the user. Make the migration a separate coherent
change before discovering new problems.
