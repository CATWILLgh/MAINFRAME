---
name: task-workflow
description: 'Universal cycle for any task that modifies code, configuration, documentation, or infrastructure — feature, bugfix, refactor, migration, ops work. Cycle: triage → recon → plan (file when ≥ 3 phases) → parallel dispatch → synthesis → independent reviewer → execution → verification → out-of-scope tickets → edge-case sweep → independent reviewer → git safety → commit → report. Plan files land in `~/.zcode/plans/<basename(cwd)>/<YYYY-MM-DD>-<topic>.md` — outside the project, not tracked by git, persistent across sessions. Adapts to both interactive sessions (uses ZCode Plan Mode / explicit plan approval when present) and unattended auto-runs (writes the plan file directly, no blocking gate). Size and urgency do not bypass the cycle; they only change which conditional steps activate.'
when_to_use: 'Trigger on any modifying task — an instruction to create, fix, add, implement, refactor, update, configure, deploy, or delete, in any language the user writes in; also multi-file refactors, bug fixes, feature work, ops changes, doc edits that change instructions. Does not run on read-only questions (what''s in this file / how does X work / where is Y / explain the architecture of Z) — those bypass the cycle entirely. ''It''s a small change'', ''it''s urgent'', ''just one file'' are not exceptions — they only change which conditional steps activate (plan file, sub-agents); verification stays unconditional and independent reviewer scales to stakes (independent review checkpoint #1 skipped only on a recon-confirmed trivial low-blast-radius change; independent review checkpoint #2 stays bar a guarded pure-mechanical edit).'
---

<!-- Generated from MAINFRAME hub (core/skills/task-workflow/SKILL.md) — do not edit. -->
...
Size and urgency are not exceptions. "It's a small change", "it's urgent", "just one file" do not skip steps. They only change which *conditional* steps run — plan file is conditional on ≥ 3 phases; sub-agent dispatch is conditional on recon scope; verification stays unconditional. independent reviewer scales to stakes the same way `decision-reviewer` does (6a): independent review checkpoint #1 (before-writing) is skipped only on a **recon-confirmed** trivial change — low blast radius (not shared / contract / auth / security, no dependents to break), reversible, obvious approach — decided *after* recon (so blast radius is known, not guessed) and **recorded** in the plan / report; independent review checkpoint #2 (before-done) stays unconditional bar one guarded pure-mechanical exception (Step 12). Size alone never triggers the skip — only recon-confirmed low blast radius does.
...
1. **Discussion / planning** — the user is in the loop (before a `/goal`, an explicit plan approval approval, or an explicit "go"). Widen coverage with recon (dispatch sub-agents), surface forks, and ask **decision-level** questions through the structured question interaction. There is **no cap on the number of questions** — drive by coverage, not a count (5 for a narrow task, 15-20 over hours for a deep one are both right). The phase stays open while a decision-level unknown remains — a fork that changes the outcome and only the user can resolve — and closes when only engineering details are left; those resolve during execution or become tickets. Recon sub-agents exist to surface the decision-level unknowns before execution starts.
...
**Boundary** = `/goal` set, plan approved via explicit plan approval, or an explicit "go / run it". Discussion can happen even with the auto-mode UI selected — what matters is whether the user is present to answer, not the mode flag.
...
Read [flow.md](flow.md) when a step sends you backward — an independent reviewer turn-back, a re-dispatch, the edge-case loop, a stop-condition cap — and you need to know where the cycle resumes. It is the full control-flow map of every turn-back; the numbered steps below are the forward path.
...
- The surrounding chain (callers, schemas, configs, usage sites) arrives as a read-only search sub-agent digest (ZCode: `Explore`) with `file:line` citations. Wholesale self-reading parks thousands of raw lines in the main context until compaction; the digest carries the same decisions at a fraction of the weight. One quick targeted read may stay inline; two or more exploration reads is already a dispatch.
...
At the same threshold, also seed a persistent todo checklist (ZCode: a persistent checklist) with the cycle's drop-prone checkpoints — recon, independent review checkpoint #1, TDD, verify, edge-case sweep, independent review checkpoint #2, out-of-scope tickets, commit — and mark each as it lands. In a long run the persistent, self-maintained list is the guard against silently dropping a mandatory step; it tracks what you have done, where the skill body only states what to do.

### 4. Parallel dispatch of investigation sub-agents

Independent investigation tasks dispatch in **one message** with multiple sub-agent calls (ZCode: ZCode subagent dispatch). Sequential calls in main context waste turns and fill the parent context with raw tool output.

Default to background fan-out where offered (ZCode: background subagent dispatch) — several agents, or any long-running one: interleaved foreground replies are hard to trace; read each result on its completion notification. Keep foreground only for a single quick agent whose result gates the very next step.
...
### 6. Decision review → Independent review checkpoint #1 (before substantive work)

Once a leading approach exists from synthesis, gate it before any writing or large dispatch. Two checks, **in this order** — the deep review first, the independent reviewer last:

**6a. High cost-of-being-wrong → dispatch `decision-reviewer` FIRST.** When the synthesised approach is an architecture choice, hard-to-reverse, broad-blast-radius, or expensive-to-undo decision, dispatch the `decision-reviewer` agent on that approach **before** the independent reviewer call. It is adversarial, grounded, and fed only the artifact — so the prompt must carry the chosen approach, the alternatives weighed, the load-bearing assumptions, and the affected files/paths (it sees **only your prompt**, not this session; a one-line "review this" starves it). Conditional on stakes — a low-stakes change skips 6a and goes straight to 6b. Do **not** fold this into the independent reviewer call; they are different checks (independent reviewer: holistic, sees your framing; decision-reviewer: adversarial, artifact-only).

**6b. Then an independent review subagent as the final checkpoint — conditional on stakes, mirroring 6a.** Call an independent review subagent after synthesis **and after any decision-review** — so the independent reviewer sees the reviewer's verdict in the transcript and makes the last call before substantive work: proceed, or turn back to further investigation / redesign. **Skip this before-writing call only on a recon-confirmed trivial change** — low blast radius (not shared / contract / auth / security, no dependents to break), reversible, obvious approach. This is Step 6, so recon (Step 2) has already established the blast radius — the skip is evidence-based, not a guess; **record it** (one line in the plan / report) so an auto-mode mis-call is auditable. Any doubt, or anything above trivial → call it. independent review checkpoint #2 (Step 12) runs regardless.

- Round cap: 3. If round 3 still surfaces new material — the approach is wrong; stop, escalate to user, do not run a 4th round.
- A critical independent reviewer finding → revise plan / approach, re-call.
- A passed independent reviewer → proceed.

**independent reviewer unavailable** (absent tool, pairing failure, runtime without it) — substitute and record it: checkpoint #1 → a **fork subagent** as stand-in independent reviewer where forking exists (inherits the conversation, parent model, cost), else `decision-reviewer` with a self-contained prompt (approach, alternatives, assumptions, files — it sees only the prompt); checkpoint #2 → a **fresh reviewer** on the diff (independence is the point, no fork).

### 7. Approval (interactive) / proceed (auto)

- **Interactive + plan file exists:** wait for the plan-approval gate (ZCode: explicit plan approval, whose `allowedPrompts` captures the granted permissions); without such a gate, present the plan and await an explicit go. The approval is the execution authorization — no extra "when to start?" turn.
...
Specialised agents — pick by the project's stack. If the project has a `AGENTS.md` listing specialised agents (e.g. `backend-python-developer`, `enterprise-react-developer`), use those. Otherwise dispatch a `general-purpose` sub-agent with explicit constraints.
...
### 12. Independent review checkpoint #2 (mandatory before declare-done)

Before declaring the task complete — an independent review subagent once more on the finished result. One round; this is validation, not re-design. **This call is unconditional, with one guarded exception: a pure-mechanical edit** — typo, rename, comment, or formatting with zero logic — **may skip independent reviewer entirely, but only when both guards hold:** the change touches no exported / public identifier (nothing external can consume it), **and** no instruction-bearing text (`AGENTS.md`, a skill, or an agent definition — those change agent behaviour, so they are never "mechanical"). Fail either guard → independent review checkpoint #2 runs.

If the independent reviewer surfaces a new issue at this stage — verify it against the actual change. A finding the independent reviewer missed during #1 but caught at #2 is worth fixing; a conflict between independent reviewer and primary-source evidence is worth one reconcile call.
...
When the change is ready and verified — invoke [`git-conventional-commits`](../git-conventional-commits/SKILL.md). It resolves the description language from the repo (explicit `AGENTS.md` directive → existing commit history → English default), not from the conversation. English `type/scope/!` always, identifier names in backticks, body as bullet list, mixed changes split into atomic commits.
...
| Excuse | Reality |
|---|---|
| "It's a small change, independent reviewer isn't needed" | independent review checkpoint #2 stays (bar a guarded pure-mechanical edit, Step 12); independent review checkpoint #1 is skipped only on a **recon-confirmed** low-blast-radius change — decided after recon, recorded, never on the *feeling* that it's small. The gate is recon-established blast radius, not size. |
...
| Loop | Max rounds |
|---|---|
| Independent review checkpoint #1 (on approach) | 3 |
| Independent review checkpoint #2 (final) | 1 |
| Edge-case sweep | 1 |
...
- [`severity-calibration`](../severity-calibration/SKILL.md) — ranking findings honestly (used in independent reviewer responses and reports).
...
- [`decision-reviewer`](../../agents/decision-reviewer.md) — adversarial grounded review of a high-stakes approach (Step 6a, before the independent reviewer checkpoint; conditional on cost-of-wrong).
