...
<!-- Generated from MAINFRAME hub (core/skills/code-audit/SKILL.md) — do not edit; regenerate via ./install.sh --codex. -->
...
testing & observability. Launch one parallel explicit read-only sub-agent per dimension in a single message, each scoped to the target. Give each the
