#!/usr/bin/env python3
"""Classify Git operations by their repository authority boundary."""
...
def repository_mutation_reason(command: str, *, depth: int = 0) -> tuple[str, str] | None:
    ...
    for segment in _segments(tokens):
        ...
        if subcommand == "update-index" or (
            ...
            return "ask", (
                "Index changes must use the reviewed Git "
                "delivery path."
            )
...
def authority_decision(command: str) -> tuple[str | None, str | None]:
