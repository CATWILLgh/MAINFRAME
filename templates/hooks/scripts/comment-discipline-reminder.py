...
- Extraction-FP (calling non-comment text a comment) — handled by
  ...
  Python (ast + tokenize) and conservative elsewhere, with a line-start-only
...
Findings are advisory. Dependency and execution failures exit nonzero so the
native binding can report the missing check; stdlib only.
"""
