...
- a server verified to run entirely on this machine, not through a tunnel,
  proxy, or remote endpoint, is disposable local test infrastructure within
  the assigned task; create, migrate, truncate, reset, or drop its test
  databases and schemas without another approval step, unless the caller
  imposed a narrower boundary. Remote, shared, staging, and production
  instances never inherit that authority;
...
- outside that verified disposable test boundary, treat `VACUUM FULL`,
  failover, restore, destructive DDL, and changes that can block writes as
  maintenance actions requiring the task's authority;
