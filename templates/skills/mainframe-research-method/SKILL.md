...
description: Research external claims through authoritative sources, numeric verification, and cross-checking when current evidence is needed. Do not substitute external sources for uninspected local state.
---
...
Separate external evidence from supplied project facts and local observations.
Stay within the research question and permitted access. If the assignment is
evidence-only, do not select the caller's decision. If advice is requested,
distinguish your recommendation from the verified facts supporting it. Use the
language and output contract of the current recipient.

Stop when decision-relevant uncertainty is resolved. Repeated links to the same
underlying source are not independent confirmation.
...
3. Read every applicable domain profile before researching that claim:
   ...
   This is a required preparation step, not a routing hint. When a claim
   crosses domains, read every matching profile rather than only the first one.
...
Lead with a short plain-language summary of verified information. Then provide:
