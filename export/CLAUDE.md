...
- Reply in Russian using plain language a non-technical reader can follow. Avoid anglicism calques when a normal Russian word exists — write «уточнение» not «refinement», «вывод» not «verdict», «граница» not «boundary». Calques that have entered Russian as standard words («небоскрёб», «холодная война») are exempt — they are already Russian. English stays only inside `backticks` for identifiers, paths, commands, and error codes; project-internal terms (ADR, skill, hook) stay as-is.
- Do not code-switch mid-sentence — English glue words and rusified English verbs are banned outside `backticks`. Specific patterns that leak in routine answers and their Russian replacements: «per принципу» → «согласно принципу / по принципу»; «via X» → «через X»; «inline» → «на месте / прямо сейчас»; «fix inline» → «починить на месте»; «surface'ю / surface в…» → «вынесу / отмечу»; «в backlog» → «в отложенное / в очередь»; «stage» → «отложить / подготовить без активации»; «через workflow» → «через процесс / через рабочий поток»; «ack / acknowledge» → «подтвердить»; «trade-off» → «компромисс»; «skip» → «пропустить»; «overrun» → «превышение»; «leak» → «протекает / проскакивает». Even when the English word feels shorter or more precise to you, in a Russian-reader context it breaks parsing flow and forces the reader to switch languages. Backticks are the only place English stands alone outside Russian grammar.
...
- For new behavior requiring guarantee: follow Red → Green → Refactor.
- Default to fast, isolated tests (unit, in-memory) that run without external services. Reach for integration tests only when a cross-boundary contract cannot be verified at unit level; reach for end-to-end only when the user-facing path itself is the contract being verified. The pyramid shape — more tests at faster levels — is the default; deviating from it requires a stated reason. Spending tokens on the right test now is the cost of a regression you do not pay later. For the level decision and anti-pattern check — use the `testing-strategy` skill; treat tests as part of the change, not as a follow-up step.
...
- For file-based subagents in `~/.claude/agents/`, default frontmatter discipline: narrow `tools:` allowlist + `maxTurns: N` cap + `permissionMode: plan` or `dontAsk` when needed. These are the verified hard knobs in the Claude Code CLI surface; do not rely on prompt-only "budget" instructions for runaway protection — soft enforcement empirically leaks in roughly half of runs.
- When invoking a one-off subagent via the Agent tool (no file-based definition), still cap its work explicitly in the prompt. Pair an ordinal cap ("at most 3 searches"), a structured return label ("return BUDGET_EXHAUSTED after the cap"), and an unconditional return clause ("whether or not you found the answer"). Hedges like "try to limit" do not constrain the model.
...
- When the runtime surfaces specialized `subagent_type`s with descriptions matching the task, dispatch to one of those instead of writing a custom prompt for `general-purpose`. Specialized agents already encode the tools allowlist, turn cap, model choice, and return format that an ad-hoc prompt would re-derive poorly. If the runtime does not surface a match, write a one-off — but check first; description-based auto-dispatch is the documented mechanism (per Claude Code `sub-agents` docs), and the prose listing visible in the Agent tool description is not always exhaustive.
- Pick the right model for the task: cheaper models for lookups and research, stronger models only when the task genuinely needs deeper reasoning.
