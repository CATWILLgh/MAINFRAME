...
machine-generated comment headers suppress both checks without treating marker
strings in human-maintained generators as headers; .sql is excluded from the
file-length check (FILE_LENGTH_EXTENSIONS carve-out); the stop_hook_active loop
guard is honored; a short file stays silent.
"""
...
def test_new_long_generated_file_is_silent():
    root = _mk_repo()
    src = (
    ...
    _add_new_file(root, "generated_client.py", src)
    assert _run_hook({"cwd": root}).strip() == ""


def test_new_long_human_generator_file_still_fires_note():
    root = _mk_repo()
    src = (
    ...
    _add_new_file(root, "build_client.py", src)
    out = _run_hook({"cwd": root})
    assert "build_client.py" in out and "405 lines" in out


def test_new_long_human_docstring_mention_still_fires_note():
    root = _mk_repo()
    src = (
        '"""\n'
        "Human-maintained client helpers.\n"
        "\n"
        "This module compares output generated by Example Tool with fixtures.\n"
        '"""\n'
        + _long_lines(397)
    )
    _add_new_file(root, "client_helpers.py", src)
    out = _run_hook({"cwd": root})
    assert "client_helpers.py" in out and "402 lines" in out
