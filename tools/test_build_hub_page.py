...
def _fixture_repo():
    ...
    _write(os.path.join(root, "dist/claude-code/plugin/skills/surface-ticket/SKILL.md"),
           "---\nname: surface-ticket\nuser-invocable: false\n"
           "description: Capture a problem as a ticket.\n"
           "when_to_use: When a problem will not be fixed now.\n---\n\nbody\n")
    _write(os.path.join(root, "dist/claude-code/plugin/skills/task-workflow/SKILL.md"),
           "---\nname: task-workflow\nuser-invocable: false\n"
    ...
    _write(os.path.join(root, "dist/claude-code/plugin/hooks/hooks.json"), json.dumps({
        "hooks": {
            "Stop": [{"matcher": "*", "hooks": [
    ...
    _write(os.path.join(root, "dist/claude-code/plugin/hooks/scripts/memory-reminder.py"),
...
def _fixture_export(root):
    """Settings + misc-layer artifacts under dist/ for the config collectors."""
    _write(os.path.join(root, "dist/claude-code/settings.json"), json.dumps({
        "env": {"FOO": "1"},
        "permissions": {
    ...
    _write(os.path.join(root, "dist/claude-code/output-styles/explanatory-concise.md"),
           "---\nname: Explanatory Concise\n---\n\n# Explanatory Concise\n\n"
           "Concise with teaching insights.\n")
    _write(os.path.join(root, "dist/claude-code/templates/credentials-index.md"),
           "# Credentials index\n\nTemplate for a per-project secrets map.\n")
    os.makedirs(os.path.join(root, "dist/claude-code/rules"), exist_ok=True)
    os.makedirs(os.path.join(root, "dist/claude-code/plugin/commands"), exist_ok=True)
...
def _fixture_health(root):
    ...
    _write(os.path.join(root, "dist/claude-code/plugin/skills/alpha/SKILL.md"),
           "---\nname: alpha\n---\n\nSee [`ghost`](../ghost/SKILL.md).\n")
    _write(os.path.join(root, "dist/claude-code/plugin/skills/anchor/SKILL.md"),
           "---\nname: anchor\n---\n\nSee [`alpha`](../alpha/SKILL.md).\n")
    _write(os.path.join(root, "dist/claude-code/plugin/skills/beta/SKILL.md"),
           "---\nname: beta\n---\n\nno refs here\n")
    _write(os.path.join(root, "dist/claude-code/plugin/agents/gamma.md"),
           "---\nname: gamma\nskills:\n  - delta\n---\n\nbody\n")
    _write(os.path.join(root, "dist/claude-code/plugin/hooks/hooks.json"), json.dumps({"hooks": {
    ...
    _write(os.path.join(root, "dist/claude-code/plugin/hooks/scripts/present.py"),
           '"""Present hook."""\nx = 1\n')
    return root
...
def test_compute_health_all_resolved_is_empty():
    root = tempfile.mkdtemp()
    _write(os.path.join(root, "dist/claude-code/plugin/skills/solo/SKILL.md"),
           "---\nname: solo\n---\n\nSee [`peer`](../peer/SKILL.md).\n")
    _write(os.path.join(root, "dist/claude-code/plugin/skills/peer/SKILL.md"),
