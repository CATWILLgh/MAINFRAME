...
A familiar filename alone is insufficient. Preserve unrelated settings and
user edits; reconcile conflicting user intent with the operator instead of
overwriting it. Existing approval applies throughout the run; do not request
...
mechanism may be processed with no installation; report that omission. A
failed check, unknown support, missing authority, or pending required reload
...
Use [the adaptation points](../templates/adaptation-points.md) wherever a native
